...
const RegistersModal = ({ buttonStyles }: modalProps) => {
  ...
  const fetchEquipos = async () => {
    const response = await fetch(`${import.meta.env.VITE_API_URL}/api/equipos`);
  ...
  const handleSubmit = async (event: { preventDefault: () => void }) => {
    ...
    try {
      if (addingNewEquipo) {
        ...
        const responseEquipo = await fetch(`${import.meta.env.VITE_API_URL}/api/equipo`, {
          method: "POST",
          headers: {
      ...
      const responseReporte = await fetch(`${import.meta.env.VITE_API_URL}/api/reporte`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (addingNewEquipo) {
        ...
        if (foto) {
          ...
          const imageResponse = await fetch(
            `${import.meta.env.VITE_API_URL}/api/equipo/${id_equipo}/img`,
            {
              method: "POST",
              body: formData,
            },
              ...
              >
                <div>
                  <FormControl fullWidth>
                    ...
                    >
                      <MenuItem value={"NO_NECESARIO"}>
                        NO_NECESARIO
                      </MenuItem>
                      <MenuItem value={"POR_RESOLVER"}>
                      POR_RESOLVER
                      </MenuItem>
                    ...
                    <div>
                      <div>
                        <FormControl fullWidth>
                          ...
                          >
                            {equipos.slice(0,100).map((equipo: Equipo) => (
                  ...
                  {addingNewEquipo == true ? (
                    <div>
                      ...
                      <FormControl fullWidth>
                        ...
                        >
                          <MenuItem value={"REPARABLE"}>Bueno</MenuItem>
                          <MenuItem value={"IRREPARABLE"}>Malo</MenuItem>
                        </Select>
                      </FormControl>
